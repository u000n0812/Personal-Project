...
| 답변이 느림 | 13장 "답변 생성이 느릴 때" 참고. 요약: 더 작은 모델(`qwen2.5:3b-instruct`), 답변 길이 "짧게", Top-K 축소, GPU 사용 여부 확인(`ollama ps`) |
...
**검색(bge-m3)과 답변 생성(qwen2.5 등)은 근본적으로 속도가 다릅니다.** bge-m3는 문장을
벡터로 바꾸는 한 번의 순전파(forward pass)만 하지만, 대화형 LLM은 답변을 한 글자씩(정확히는
토큰 단위로) 순차적으로 만들어냅니다. 같은 하드웨어에서도 후자가 훨씬 느린 게 정상입니다.

프로그램이 자동으로 하는 최적화(추가 설정 필요 없음):
- **답변 길이에 맞춰 생성 토큰 상한을 지정**합니다 — "짧게"를 선택하면 애초에 짧게 끝나도록
  제한합니다(모델이 스스로 멈추기를 기다리지 않습니다).
- **실제 쓰는 만큼만 문맥 크기(context)를 잡습니다** — 지정하지 않으면 Ollama가 모델의
  기본 상한(대개 필요한 것보다 훨씬 큼)을 그대로 써서 매 토큰 계산이 느려집니다.
- **질문 사이에 모델을 메모리에 계속 띄워둡니다**(30분) — 기본값(5분)보다 길게 잡아, 답변을
  읽는 동안 모델이 내려갔다가 다음 질문에 다시 올라오는 지연을 없앱니다.

직접 조정할 수 있는 것:
1. **더 작은 모델 사용** — 가장 효과가 큽니다. `ollama pull qwen2.5:3b-instruct` 후
   Settings → LLM 모델을 바꾸세요. 7b보다 2~3배 빠른 대신 답변 품질은 약간 낮아집니다.
2. **답변 길이를 "짧게"로** — 생성할 토큰 수 자체가 줄어듭니다.
3. **검색 문서 수(Top-K)를 줄이기** — 모델이 읽어야 할 입력이 줄어 첫 토큰이 더 빨리 나옵니다.
4. **GPU 사용 여부 확인** — CPU만으로 7b 모델을 돌리면 매우 느립니다(수십 초~1분 이상도
   흔합니다). 터미널에서 `ollama ps`를 실행했을 때 `PROCESSOR` 열에 `100% GPU`가 아니라
   `CPU`가 보이면, GPU를 못 쓰고 있다는 뜻입니다. NVIDIA GPU가 있다면 최신 드라이버 설치
   여부를 확인하세요. GPU가 없는 PC라면 3번(`qwen2.5:3b-instruct`)이 현실적인 대안입니다.

> 검색(Embedding)은 원래도 빠릅니다 — bge-m3를 LLM 모델로 착각해서 "빨랐다"고 느끼신 거라면,
> 그건 애초에 답변을 생성한 게 아니라 검색만 하고 실패한 상태였을 가능성이 높습니다(이전 대화 참고).
